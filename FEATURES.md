...
| **Revenue Heat Map: State × COVID Phase** (`HeatMap`) | Revenue magnitude across two categorical dimensions at once | Two categorical axes × one measure → **matrix with a sequential ramp** (light→dark = low→high, perceptually ordered). Clicking a cell cross-filters the whole dashboard — the mantra's *zoom and filter* made literal. |
...
| **Project Progress Timeline** (`ProjectTimeline`) | Project milestones against real MCO/CMCO/RMCO boundaries | Temporal events → position along a time axis; the phase bands give every milestone its pandemic context. |
...
| **Scatter Plot Matrix** (`ScatterMatrix`, Correlation tab) | Pairwise correlation across 6 numeric variables | The canonical **bivariate-relationship** form; per-cell sampling keeps it responsive; color = COVID phase with a legend, so the pandemic's effect on each relationship is visible. |
