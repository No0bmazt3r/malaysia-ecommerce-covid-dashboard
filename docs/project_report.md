...
- **URL State Persistence:** The active filter state is continuously serialized into the browser's URL (e.g., `?f=...`). This allows any deeply nested analysis (e.g., "Sabah during MCO looking at Electronics") to be bookmarked, shared, and instantly reproduced.
